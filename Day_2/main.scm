(define (sum-lsts lsts)
  (define (iter lsts sum)
    (if (null? lsts)
        sum
        (iter (cdr lsts) (+ sum (safe-permutations (car lsts))))))
  (iter lsts 0))

(define (safe-permutations lst)
  (define (permutations length iter)
    (cond
     ((= iter length) 0)
     ((> (safe? (remove-elem lst iter)) 0) 1)
     (else (permutations length (+ 1 iter)))))
  (if (= 0 (safe? lst))
      (permutations (list-length lst) 0)
      1))
...
(define (remove-elem lst nr)
  (if (= nr 0)
      (cdr lst)
      (cons (car lst) (remove-elem (cdr lst) (- nr 1)))))

(define (safe? lst)
  (define (iter lst asc prev removed)
    (cond
     ((null? lst) 1)
     ((> (isBad lst asc prev) 0) 0)
     (else (iter (cdr lst) asc (car lst) removed))))
  (iter (cdr lst) (ascending? (car lst) (cadr lst)) (car lst) 0))

(define (isBad lst asc prev)
  (+ (deltaBiggerThanThree lst prev)
     (deltaIsZero lst prev)
     (ascendingChanged asc lst prev)))

(define (deltaIsZero lst prev)
  (if (= 0 (delta (car lst) prev))
      1
      0))

(define (deltaBiggerThanThree lst prev)
  (if (> (delta (car lst) prev) 3)
      1
      0))

(define (ascendingChanged asc lst prev)
  (if (not (= asc (ascending? prev (car lst))))
      1
      0))
...
(newline)
(display (safe? '(7 6 4  2 1)))

(newline)
(display (safe? '(1 2 7 8 9)))

(newline)
(display (safe? '(9 7 6 2 1)))

(newline)
(display (safe-permutations '(1 3 2 4 5)))

(newline)
(display (safe? '(8 6 4 4 1)))

(newline)
(display (safe? '(1 3 6 7 9)))
